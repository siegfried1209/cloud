 ...
 document.getElementById('vlanform_eperu').addEventListener('submit', function(event) {
    event.preventDefault(); // Evita el envío del formulario de forma tradicional

    const vlan_peru = document.getElementById('vlan_eperu').value;
    const int_eperu = document.getElementById('int_eperu').value;

    const data = {
        vlan_id_peru: vlan_peru,
        interface_peru: int_eperu
    };

    fetch('https://192.168.1.39:3000/vlan_eperu', {
        method: 'POST',
        headers: {
...
function obtenerDatosMac() {
    fetch('https://192.168.1.39:3000/datos-mac-eperu')
        .then(response => {
            if (!response.ok) {
